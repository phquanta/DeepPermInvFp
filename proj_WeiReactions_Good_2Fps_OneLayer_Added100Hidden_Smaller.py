...
def funx(i,maxL,Test=False):
       if Test: 
        if(i<maxL):
            ...
            return fp1_test[:,i-maxL]
        else:
            return X_test[:,i-2*maxL]
        ...
        if(i<maxL):
           ...
           return fp_train[:,i]
            ...
            return fp1_train[:,i-maxL]
        else: 
            return X_train[:,i-2*maxL]
...
#           callbacks=[es,reduce_lr])
        ...
        model.fit([funx(i,maxL) for i in range(2*maxL)],y_train, epochs=100,batch_size=100, 
           validation_data=([funx(i,maxL,Test=True) for i in range(2*maxL)],y_test),
 ...
 #          callbacks=[es,reduce_lr])
    ...
    data1=model.predict([funx(i,maxL,Test=True) for i in range(2*maxL)])
    data2=model.predict([funx(i,maxL,Test=False) for i in range(2*maxL)])
